...
from Utils.utils import report_stats, report_bleu, report_rouge
...
class Trainer(object):
    ...
    def validate(self, valid_iter, translator, builder):
        """ Validate model.
        ...
        for batch in valid_iter:
            kld_loss = 0.
            """
            outputs_ = translator.translate_batch(batch)
            batch_trans = builder.from_batch_translator_output(outputs_)
            for trans in batch_trans:
                pred_list.append(trans.pred_sents[0])
                gold_list.append(trans.gold_sent)

            del outputs_, batch_trans
            """
            normalization = batch.batch_size
